...
class StudyAgentsSystem:
    ...
    def generate_notes(self, topics: Optional[list[str]] = None, model: Optional[str] = None, user_id: Optional[str] = None, conversation_history: Optional[list[dict]] = None, topic: Optional[str] = None, user_level: Optional[int] = None, chat_id: Optional[str] = None) -> str:
        ...
        # Misma palanca que ask_question: sin ENABLE_RESPONSE_CORRECTOR=true no hay segunda pasada (mucho más rápido).
        enable_corrector = os.getenv("ENABLE_RESPONSE_CORRECTOR", "false").lower() == "true"
        if not enable_corrector:
            print("⏭️ Saltando corrector de apuntes (ENABLE_RESPONSE_CORRECTOR no está en true)")
            return notes
    ...
    def ask_question(
        ...
        **kwargs: Any,
        ...
        # Modo rápido por defecto: desactiva la segunda pasada del agente corrector
        # para reducir drásticamente la latencia en producción.
        enable_response_corrector = os.getenv("ENABLE_RESPONSE_CORRECTOR", "false").lower() == "true"
        if not enable_response_corrector:
            skip_corrector = True
        
        if skip_corrector:
            if enable_response_corrector:
                print("⏭️ Saltando corrector (flashcards / tests / ejercicios / palabras clave)")
            else:
                print("⏭️ Saltando corrector de respuesta (ENABLE_RESPONSE_CORRECTOR desactivado por defecto)")
