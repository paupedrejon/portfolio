...
export default function Hyperspeed({
  ...
  effectOptions?: typeof DEFAULT_EFFECT_OPTIONS & { colors?: Partial<typeof DEFAULT_EFFECT_OPTIONS.colors> };
}) {
  ...
  useEffect(() => {
    ...
    > = {
      ...
      deepDistortion: {
        ...
        getJS: (progress, time) => {
          ...
          const getY = (p: number) =>
            Math.pow(Math.abs(p * uPowY.x), uPowY.y) + Math.sin(p * Math.PI * uFreq.y + time) * uAmp.y;
          const distortion = new THREE.Vector3(
    ...
    class CarLights {
      ...
      init() {
        ...
        // @ts-expect-error - InstancedBufferGeometry.copy accepts BufferGeometry at runtime
        ...
        const colorObjs = Array.isArray(colors)
          ? colors.map((c) => new THREE.Color(c))
          : [new THREE.Color(colors as unknown as number)];
        ...
        for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
          ...
          aOffset.push(laneX + carWidth / 2, offsetY, offsetZ);
          aMetrics.push(radius, length, speedVal, radius, length, speedVal);
  ...
  update(time: number) {
    ...
    class LightsSticks {
      ...
      init() {
        ...
        const stickColors = options.colors.sticks;
        const colors = Array.isArray(stickColors)
          ? (stickColors as number[]).map((c) => new THREE.Color(c))
          : [new THREE.Color(stickColors)];
    ...
    type Options = typeof DEFAULT_EFFECT_OPTIONS & {
      distortion: (typeof distortions)[string];
...
class App {
  ...
  dispose() {
    ...
    const distKey = options.distortion as string;
    options.distortion = (distortions[distKey] || distortions.turbulentDistortion) as typeof options.distortion;
  ...
  return <div id="lights" ref={hyperspeedRef} className="hyperspeed-container" />;
}
