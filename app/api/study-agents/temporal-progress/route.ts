...
export async function POST(request: NextRequest) {
  try {
    ...
    const { apiKey, user_id, days } = body;
    ...
    const response = await fetch(`${FASTAPI_URL}/api/temporal-progress`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      let errorMessage = "Error al obtener progreso temporal";
    ...
    console.error("Error en temporal-progress:", error);
    return NextResponse.json(
      {
        success: false,
        error:
